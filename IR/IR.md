...
这里总结我看信息检索导论中遇到的值得总结的问题。很多问题都没有提，有一部分是纯NLP的，会另开一个，有一部分是纯粹我不会的。
...
当面临信息检索任务的时候，比如在进行文档匹配的时候，我们会设定某些阈值，例如我们可能将文档长度设定为200，当小于这个阈值的时候文档会被舍掉。

但是，还有情况是，我们会面临着，返回的文档过少的情况，这样的话，我们可以设定多个阈值，先搜寻小阈值的情况，如果文档数目不够，可以返回大阈值的情况。这样可以增加搜索引擎的效率。
...
什么样的算无序呢？比如给定20篇文档，我们返回这20篇文档中与检索结果哪条相关，哪条无关。而不考虑其排序影响。

无序检索结果评价，主要依靠准确率和召回率来评价。

准确率 = 返回结果中相关文档数目/返回结果数目

召回率 = 返回结果中相关文档数目/所有相关文档数目

一个均衡化了准确率召回率的指标是F值，一般采用F1值=2PR/(P+R)

13.衡量有序检索结果的评价指标都有哪些
-------------------------------------
...
* 准确率，召回率
* F值
* MAP，平均正确率均值。这里解释一下，我们可以发现，准确率和召回率能合成一个曲线，系统性能越突出的，对应面积越大，相应的，MAP即是这个面积。具体请参考![这里](http://blog.csdn.net/marising/article/details/6543943)
* P@K，前K个结果的正确率。
* ROC
* NCDG
* MRR，第一个结果匹配，则分数为1，第n个结果匹配，则分数为1/n，没有匹配为0。最终为所有分数之和
...
先说构建同义词词典有什么用，构建同义词词典最简单、直接的作用就是可以用作查询扩展。顾名思义，就是我们在检索比如"非常好"的时候，检索系统能够给我们返回一些与之相似的词，作为我们的候选词，例如"棒极了"。也可以在检索的时候，加入同义词，以达到增加召回率的目的。

一个最为简单的构建同义词典的做法是使用同义词词典或其他基于人工的方法。这种方法成本比较高。不过比基于统计的方法要准确一些。

基于统计的方法中，比较简单的是利用词之间的共现关系，如果这两个词共现，则认为他们是有关系的。这种方法比较类似与关联规则。

基于主题的方法，我们可以利用LSA，PLSA，LDA或者更为复杂一些的统计模型来进行构建。

基于比较新的算法，例如word2vector等方法。

总而言之，基于统计的方法主要在于计算语义相似度、或者词的主题成分上。
